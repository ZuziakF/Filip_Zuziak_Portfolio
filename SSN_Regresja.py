...
def load_and_prep_data(filepath):
    ...
    y = df['Target_Regresja'].values.reshape(-1, 1)
    
    return X, y

def run_experiment(X, y, test_split, layers, lr, activation, epochs, repeats=5):
    ...
    y_mean, y_std = np.mean(y_train_raw, axis=0), np.std(y_train_raw, axis=0)
    ...
    for r in range(repeats):
        nn = NeuralNetworkRegression(layer_sizes=layers, learning_rate=lr, activation=activation)
    ...
    best_train = np.min(train_errors)
    avg_test = np.mean(test_errors)
    best_test = np.min(test_errors)
...
if __name__ == "__main__":
    ...
    for exp_name, values in experiments.items():
        ...
        for val in values:
            ...
            avg_tr, best_tr, avg_te, best_te = run_experiment(X, y, split, layers, lr, act, epochs, repeats=5)
            
            print(f"{label:<15} | {avg_tr:<15.4f} | {best_tr:<18.4f} | {avg_te:<15.4f} | {best_te:.4f}")
